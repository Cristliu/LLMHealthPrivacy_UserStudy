...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('send-button').addEventListener('click', function() {
        ...
        if (!userMessage) {
            ...
            // 如果用户点击次数超过10次，显示提醒
            if (consultCount > 10) {
                alert('The number of consultations has reached 10, please end the consultation and proceed to the questionnaire');
                // 清空输入框
    ...
    document.getElementById('end-button').addEventListener('click', function() {
        ...
        if(consultCount === 0){
            alert("Please complete at least one consultation first");
            return;
        }
        // 使用fetch API向后端发送请求，告知当前会话已结束
        fetch(`/LLMHPri/end_session_en/${sessionId}/`, { // 使用模板字符串构造URL，确保 sessionId 被正确插入
            method: 'POST',
            headers: {
...
function sendMessageToServer(message, provideInfo, age, gender, occupation) {
    ...
    fetch(`/LLMHPri/send_message_en/${sessionId}/`, {
        method: 'POST',
            headers: {
                'X-CSRFToken': getCookie('csrftoken'),
                'Content-Type': 'application/json',
...
function displayMessage(message, sender) {
    ...
    if (sender === 'gpt') {
        ...
        typeMessage(textContentP, message, 25); // 使用打字效果
...
function postToQNSurvey() {
    // 创建一个表单
    var form = document.createElement("form");
    form.setAttribute("method", "post");
    form.setAttribute("action", "/LLMHPri/qnsurvey/");
...
function showWaitingMessage() {
    ...
    waitingMessage.textContent = 'Please wait patiently for the response...';
...
function showConsultation(type) {
    ...
    // 如果没有选中任何选项，则显示错误消息
    if (!isSelected) {
        alert('Please choose whether or not to provide information first!');
        return; // 阻止进一步操作
    }
    ...
    if (provideInfo === 'yes' && !occupation) {
        alert('Please select your Occupation or Identity');
        return; // 阻止进一步操作
    }
...
function loadScriptBasedOnSessionId_QNSurvey() {
    // 检查是否在survey.html页面
